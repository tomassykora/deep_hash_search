...
class DataGenerator(object):
	...
	def __make_triplets(self):
		...
		for Id, c in enumerate(classes):
			...
			neg_dir = os.path.join(self.dataset_path, classes[(Id+1)%len(classes)])
			...
			for i, img in enumerate(imgs_pos):
				...
				class_triplets.append((c+'/'+imgs_pos[i], c+'/'+imgs_pos[(i+1) % len(imgs_pos)], classes[(Id+1)%len(classes)]+'/'+imgs_neg[i%len(imgs_neg)]))
